...
export -f printf_info
repo forall -c 'bash -c printf_info' # export 后, printf_info 才能被识别
...
# 简单缩进中的坑
function blog() {
    blog="www.zddhub.com" # 作用于为定义到文件结尾，或者显示删除变量行后
    echo "print $blog"
}
blog | indent
...
通常在保存 log 时会把标准错误输出和标准输出保存到同一个文件，使用 2>&1 实现。
    ...
    - 解释以下两个命令的区别:
      ...
      shell 实现重定向时使用函数 dup2(a,b) 将文件表述符结构体 a 的内容 copy 给 b，使得 b 含有和 a 相同的设备/文件。
...
以防受其影响。
    ...
    # 如果循环体中的命令会影响标准输入输出/标准错误输入输出时，需要重定向到新的描述符
