...
`protoc` 起初只支持谷歌自家内部使用的六种语言，后发展到[十种][Protobuf_Runtime]，其它语言由[第三方提供][Protobuf_Third_Party]。
...
protobuf 也类似，只不过关键字不同，变不到哪里去。具体的语法可以参考 [Language Guide (proto3)][Proto3_Guide]。若没有特别说明，本文内容以 proto3 为准。

我一般会用一个 CheatSheet 来快速了解一门新语言的写法，当然 protobuf 也有，这里是 Github Gist 上 proto3 的一个 [CheatSheet][Proto3_CheatSheet]，供大家参考。
...
message Message { // 2. 定义数据结构 Message，用关键字 message 表示
  int32 id = 1; // 3. 定义 id，类型为 int32. 其它 Integer 的定义类似。并定义了字段号，字段号用于编码，最小字段号为 1
  ...
  message InnerMessage {  // 8. message 可以嵌套，定义一个内部的消息
