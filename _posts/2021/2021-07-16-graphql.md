...
`GraphQL` 可以类比 `SQL(Structured Query Language)`, 都是一种查询语言，通过查询语句可以获得期望的结果。不同之处有两点：
1. `SQL` 是基于机构化的数据模型，而 `GraphQL` 是基于图的。
2. `SQL` 是从数据库查询，而 `GraphQL` 是从 `GraphQL` 服务器查询。

#### 为什么 GraphQL 基于图？

因为大多数应用程序的数据模型是基于图的。拿博客系统为例，一个作者可以撰写多篇博客，而同一个博客也可以由多个合作者功能完成，如果两个作者写过同一篇博客，那么他们互为合作者。博客和作者为这个图中的实体，属于图的节点，而实体之间的关系为图的边。特别的，实体和他们属性之间的关系也属于边，属性内容为叶子节点。如下图所示：

![Blog Data Model][Blog_Data_Model]

这是一个双向循环图，`GraphQL` 就是基于这种典型的图的数据模型，构建和查询数据的。
...
`GraphQL` 是如何来描述应用程序的数据模型的呢？它借鉴了类型系统，使用类型来描述实体，使用类型之间的关系来描述实体之间的关系。比如上面的博客系统，对应的 Schema 如下：
...
简单博客应用程序的数据模型是一张图，一张双向循环图。对于更复杂的大型应用程序，它的数据模型可能包含更多的节点和边，一次查到整张图显然是不合适的。那么 `GraphQL` 需要确定查询的入口和如何遍历图，它通过定义 `Query` 来实现。假如你想查询所有博客, 你可以
...
查询语句开头的 query 可以省略, 结果的结构和 query 结构十分相似，返回结果放在 data 底下。需要在 Schema 中定义 Query 语句来支持这种查询。如下：
...
通过给定的 Query Schema，你可以自由选择查询根节点并制定遍历策略。GraphQL 在图中遍历后，结果通过树返回。例如上述例子中，`authors -> blogs -> authors -> blogs`, 查询结果通过增加新的子节点而不是循环引用，简化了数据的抽象。

![Query Model][Query_Model]

#### 修改数据

GraphQL 是一种 API 的设计模式，API 支持增删查改，GraphQL 使用 Mutation 来支持对数据的修改，包括：
- 创建数据
- 更新数据
- 删除数据

例如, 以下 Schema 定义了对 Author 的增删更新操作：
...
在某些业务场景下，客户端需要和服务器端保持长连接，当特定 event 发生后，服务器端需要实时通知客户端。GraphQL 也支持这种情况，使用 subscription 来实现。
...
真高兴你能读到这里！上面我们已经介绍了 GraphQL 的基本概念，用法和使用场景，对 GraphQL 这个查询语言已经有一定了解，那么一个 GraphQL 查询到底会经历怎么样的过程呢？现在让我们看看执行引擎部分。
...
在实际使用中，GraphQL周边的库会帮我们做这些事情。我们只需要撰写 Query 语句。
...
除了 Schema，在服务器端最重要的就是解析函数了，它类似路由，回答了数据从哪里来的问题。
...
像之前说过的，GraphQL 从后台的图中检索出一棵树返回给前端。从 Query.authors 开始，一层一层的 resolver，直到所有查询值全部被解析出来为止。

*注：这种便于理解，实际中服务端会做各种优化。*
...
含有四个参数：
- parent: parent是父节点的返回值，包含父节点解析后的信息
- args: GraphQL 里传过来的参数
- context：context是共享数据，在多个 resolver之间共享，比如数据库的连接，认证信息等
...
GraphQL 提供了强大灵活的数据检索方案，也暴露了很多风险，如果用户恶意的使用 Query 语句，例如构造足够长的 Query 语句等，很容易拖垮服务器。一般来说，服务器端采用以下策略来避免风险。

| 安全措施 | 优点 | 缺点 |
|---|---|---|
| 设置超时 | 有效 | 在超时之前损害有可能已经完成，很难超时时间 |
| 最大查询深度 | AST 能在执行前检测出问题，拒绝该请求 | 通常不足以覆盖所有滥用的查询 |
| 查询复杂度 | 覆盖更多情况，在执行前检测并拒绝请求 | 很难实现 |
| 节流 | 能方式用户频繁访问 | 复杂 |

GraphQL非常适合客户端使用，它为客户端提供了更多的功能。但是，这种能力也使他们有可能使用非常昂贵的查询滥用 GraphQL 服务器。

有许多方法可以保护GraphQL服务器不受这些查询的影响，但是没有一种方法是万无一有的。重要的是要知道哪些选项是可用的，知道它们的限制，这样我们才能做出最好的决定!
...
第二个查询没有必要再获得作者的信息，因为相关信息的值已经在第一次查询中返回。
...
2000 年出现的 REST 因为*无状态和结构化数据* 被广泛使用，Ruby on Rails，Nodejs等框架更是进一步给行业科普了 REST 的概念。以下是 REST 和 GraphQL 的比较：
...
这里有一个很好的练习 GraphQL 的教程—— [Learn GraphQL with Apollo][Learn_GraphQL_with_Apollo]，感兴趣的同学可以私下练习。
...
[GraphQL_Excution]: /assets/images/2021-07-16/graphql-execution.png
